...
<head>
    ...
    <script>
        function modify() {
            ...
            window.location.href = "UpdateServlet?username=${user.getUsername()}&roomname=${room.getRoomname()}&rent_price=" + rent_price + "&used_water=" + used_water + "&used_electronic=" + used_electronic + "&net_price=" + net_price;
        }
...
<body>
<div id="a">
    <div style="font-size: 40px;margin-top: 40px;margin-left: 70px">房屋信息修改界面</div>
    <div id="b">
        <span>管理员:${user.getUsername()}</span><br>
        房屋名称：${room.getRoomname()}<br>
        <br>
        ...
        <button onclick="modify()">确认修改</button>
        <span>${error}</span>
    </div>
</div>
